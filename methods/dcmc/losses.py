...
def loss_pam_photometric(img_left, img_right, att, valid_mask, mask=None):
    scale = img_left.size()[2] // valid_mask[0].size()[2]

    att_right2left = att[0]                          # b * h * w * w
    att_left2right = att[1]
    valid_mask_left = valid_mask[0]                  # b * 1 * h * w
    valid_mask_right = valid_mask[1]
...
def loss_pam_cycle(att_cycle, valid_mask):
    ...
    att_left2right2left = att_cycle[0]
    att_right2left2right = att_cycle[1]
    valid_mask_left = valid_mask[0]
    valid_mask_right = valid_mask[1]

    loss = F.l1_loss(att_left2right2left * valid_mask_left.permute(0, 2, 3, 1), I * valid_mask_left.permute(0, 2, 3, 1)) + \
